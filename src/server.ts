...
async function ensureFont(fontFamily: string): Promise {
...
function drawSpaced(ctx: any, text: any, x: any, y: any, sp: any, fs: any, al: any){if(sp===0){ctx.fillText(text,x,y);return;}var s=sp*fs,tot=ctx.measureText(text).width+s*(text.length-1),sx=al==='center'?x-tot/2:al==='right'?x-tot:x,sa=ctx.textAlign;ctx.textAlign='left';var cx=sx;for(var i=0;i/ ── OverlayParams type ───────────────────────────────────────────────────────
...
async function renderPngInternal(params: RenderParams): Promise {
 ...
 rawRgba = await new Promise((resolve, reject) => {
...
async function findExistingPrintfulOrder(externalId: string): Promise {
 try {
 ...
 */
async function resolveExternalId(baseId: string): Promise {
...
async function renderConfigToBlobUrl(
 ...
 const rawCfg = await cfgRes.json() as Record;
 ...
 let styleJson: Record;
 try {
 styleJson = JSON.parse(JSON.stringify(cfg.styleJson)) as Record;
 const sources = styleJson.sources as Record> | undefined;
 if (sources) {
 ...
 styleJson = cfg.styleJson as Record;
 }
...
app.post('/fulfill', async (req: Request, res: Response): Promise => {
 if (!checkAuth(req, res)) return;
