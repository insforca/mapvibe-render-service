...
function applyFades(ctx: any, W: any, H: any, color: any, fs: any){if(fs==='none')return;var tH=Math.round(H*0.25),tg=ctx.createLinearGradient(0,0,0,tH);tg.addColorStop(0,_wa(color,1));tg.addColorStop(.4,_wa(color,.45));tg.addColorStop(.7,_wa(color,.12));tg.addColorStop(1,_wa(color,0));ctx.fillStyle=tg;ctx.fillRect(0,0,W,tH);_dr(ctx,0,0,W,tH);if(fs==='text'){var bH=Math.round(H*.25),bY=H-bH,fg=ctx.createLinearGradient(0,H,0,bY);fg.addColorStop(0,_wa(color,1));fg.addColorStop(.5,_wa(color,.5));fg.addColorStop(1,_wa(color,0));ctx.fillStyle=fg;ctx.fillRect(0,bY,W,bH);_dr(ctx,0,bY,W,bH);}else{var bH=Math.round(H*.25),bY=H-bH,bg=ctx.createLinearGradient(0,H,0,bY);bg.addColorStop(0,_wa(color,1));bg.addColorStop(.4,_wa(color,.45));bg.addColorStop(.7,_wa(color,.12));bg.addColorStop(1,_wa(color,0));ctx.fillStyle=bg;ctx.fillRect(0,bY,W,bH);_dr(ctx,0,bY,W,bH);}}
...
function textMetrics(w: any, h: any, layout: any){if(layout==='editorial'){var x=w*.06;return{cX:x,cY:h*.805,dX:x,dY:h*.840,coX:x,coY:h*.870,crX:x,crY:h*.905,al:'left',dW:120};}var cx=w*.5;return{cX:cx,cY:h*.845,dX:cx,dY:h*.875,coX:cx,coY:h*.900,crX:cx,crY:h*.930,al:'center',dW:w*.2};}
