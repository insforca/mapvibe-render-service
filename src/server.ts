...
function applyFades(ctx: any, W: any, H: any, color: any, fs: any){if(fs==='none')return;if(fs==='text'){var tH=Math.round(H*0.25),tg=ctx.createLinearGradient(0,0,0,tH);tg.addColorStop(0,_wa(color,1));tg.addColorStop(.4,_wa(color,.45));tg.addColorStop(.7,_wa(color,.12));tg.addColorStop(1,_wa(color,0));ctx.fillStyle=tg;ctx.fillRect(0,0,W,tH);_dr(ctx,0,0,W,tH);var fH=Math.round(H*.095),gH=Math.round(H*.080),fT=H-fH-gH,fg=ctx.createLinearGradient(0,fT,0,fT+gH);fg.addColorStop(0,_wa(color,0));fg.addColorStop(.18,_wa(color,.08));fg.addColorStop(.34,_wa(color,.22));fg.addColorStop(.5,_wa(color,.46));fg.addColorStop(.65,_wa(color,.70));fg.addColorStop(.8,_wa(color,.88));fg.addColorStop(.92,_wa(color,.97));fg.addColorStop(1,color);ctx.fillStyle=fg;ctx.fillRect(0,fT,W,gH);ctx.fillStyle=color;ctx.fillRect(0,H-fH,W,fH);_dr(ctx,0,fT,W,gH+fH);} else {var bH=Math.round(H*.28),bY=H-bH,bg=ctx.createLinearGradient(0,H,0,bY);if(process.env.DISABLE_EASEIN_FADE==='true'){bg.addColorStop(0,_wa(color,1));bg.addColorStop(.4,_wa(color,.45));bg.addColorStop(.7,_wa(color,.12));bg.addColorStop(1,_wa(color,0));}else{bg.addColorStop(0,_wa(color,1));bg.addColorStop(.15,_wa(color,.997));bg.addColorStop(.30,_wa(color,.973));bg.addColorStop(.48,_wa(color,.890));bg.addColorStop(.62,_wa(color,.762));bg.addColorStop(.74,_wa(color,.594));bg.addColorStop(.84,_wa(color,.407));bg.addColorStop(.91,_wa(color,.247));bg.addColorStop(.96,_wa(color,.111));bg.addColorStop(.99,_wa(color,.030));bg.addColorStop(1,_wa(color,0));}ctx.fillStyle=bg;ctx.fillRect(0,bY,W,bH);_dr(ctx,0,bY,W,bH);}}
...
function textMetrics(w: any, h: any, layout: any){if(layout==='editorial'){var x=w*.06;return{cX:x,cY:h*.82,dX:x,dY:h*.872,coX:x,coY:h*.897,crX:x,crY:h*.921,al:'left',dW:120};}var cx=w*.5;return{cX:cx,cY:h*.855,dX:cx,dY:h*.907,coX:cx,coY:h*.934,crX:cx,crY:h*.961,al:'center',dW:w*.2};}
...
interface FulfillBody {
  ...
  variantId:        number;
  catalogVariantId?: number;  // deprecated — v2 requires global catalog IDs, distinct from store variant IDs; not used
  label:            string;
  quantity:         number;
  pngUrl?:          string;
  configUrl?:       string;
...
app.post('/fulfill', async (req: Request, res: Response): Promise<void> => {
  ...
  if (!externalId || !recipient || !variantId || !label || !quantity) {
    res.status(400).json({ error: 'Missing required fields: externalId, recipient, variantId, label, quantity' });
    return;
  }
  if (!pngUrl && !configUrl) {
  ...
  const giftMessage = gift?.message
    ...
    // NOTE: Printful v2 API is intentionally skipped.
    // v2 requires `catalog_variant_id` (global catalog IDs), but SKU_MAP stores
    // Printful store variant IDs (v1 API). Passing a store ID to v2 always returns
    // a validation error. v1 is proven-reliable and the correct path for now.
    const v1Payload = {
    ...
    try {
      const pfRes = await fetch(`${PRINTFUL_API_V1}/orders`, { method: 'POST', headers: pfHeaders, body: JSON.stringify(v1Payload) });
      const pfData: any = await pfRes.json();

      if (pfRes.ok) {
        const orderId = pfData.result?.id ?? pfData.data?.id;
        console.log(`[fulfill] Printful order created: ${orderId} for ${effectiveExternalId} (base: ${externalId})`);
        return;
      }
