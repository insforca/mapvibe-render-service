...
function textMetrics(w: any, h: any, layout: any, cfs?: any, ctFS?: any, coFS?: any){if(layout==='editorial'){var x=w*.06;return{cX:x,cY:h*.82,dX:x,dY:h*.872,coX:x,coY:h*.897,crX:x,crY:h*.921,al:'left',dW:120};}var cx=w*.5,cY=h*.855;if(cfs&&ctFS&&coFS&&process.env.DISABLE_MEASURED_GAP!=='true'){var gap=Math.min(Math.max(cfs*.55,h*.030),h*.065),dY=cY+gap,coY=dY+ctFS*.52,crY=coY+coFS*.85;return{cX:cx,cY,dX:cx,dY,coX:cx,coY,crX:cx,crY,al:'center',dW:w*.2};}return{cX:cx,cY,dX:cx,dY:h*.907,coX:cx,coY:h*.934,crX:cx,crY:h*.961,al:'center',dW:w*.2};}
