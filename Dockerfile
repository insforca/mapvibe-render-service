...
# Inject real ICU 74 libraries from Ubuntu 24.04 Noble (official archive, HTTPS)
RUN curl -fsSL https://archive.ubuntu.com/ubuntu/pool/main/i/icu/libicu74_74.2-1ubuntu3_amd64.deb -o /tmp/libicu74.deb \
  && dpkg-deb --extract /tmp/libicu74.deb /tmp/icu74 \
  && ls /tmp/icu74/usr/lib/x86_64-linux-gnu/libicu*.so.74 \
  && cp -a /tmp/icu74/usr/lib/x86_64-linux-gnu/libicu*.so.74* /usr/lib/x86_64-linux-gnu/ \
  && ldconfig \
  && rm -rf /tmp/icu74 /tmp/libicu74.deb \
  && ldconfig -p | grep 'libicuuc.so.74'
